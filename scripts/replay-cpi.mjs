...
import { replayCpiHistory } from '../src/replay/cpi-history.js';
...
const registryPath = path.join(root, 'data', 'cpi', 'headline-yoy-releases.json');
...
const result = await replayCpiHistory({ fredAdapter, kalshiAdapter, releases });
...
const outputPath = process.env.REPLAY_OUTPUT
  ...
  : path.join(outDir, `cpi-${result.modelVersion}-${new Date().toISOString().replace(/[:.]/g, '-')}.json`);
